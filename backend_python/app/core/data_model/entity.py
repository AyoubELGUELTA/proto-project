...
class EntityModel(DescriptiveModel):
    ...
    Attributes:
        ...
        review_status: Tracks the curation state of the entity.
    """
    ...
    @model_validator(mode="before")
    @classmethod
    def set_default_slug(cls, data: Any) -> Any:
        if isinstance(data, dict) and "slug" not in data:
            data["slug"] = slugify_entity(data.get("title", "unknown"))
        return data


    @model_validator(mode="before")
    @classmethod
    def handle_nan(cls, data: Any) -> Any:
        """It prevents the type error, when "nan" (float in dataframes) is None in reality."""
