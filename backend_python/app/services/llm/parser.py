...
class LLMParser:
    ...
    def to_dataframes(
                ...
                # --- CASE: RELATIONSHIP ---
                    ...
                    try:
                        rel = RelationshipModel(
                            source_id=t[1],# We temporarily put the names as source ids
                            target_id=t[2],# Same
                            source_slug=slugify_entity(t[1]),
                            target_slug=slugify_entity(t[2]),
                            description=t[3],
        ...
        rel_df = pd.DataFrame(all_relationships) if all_relationships else pd.DataFrame(
            columns=["source_id", "target_id", "source_slug", "target_slug", "weight", "description", "source_ids"]
        )
